...
public class MyFileRenamePolicy implements FileRenamePolicy{
	...
	/*
	 * 반드시 미완성된 reanme메소드를 오버라이딩해서 구현해야함.
	 * 기존의 파일을 전달받아서 파일명 수정작업후에 수정된파일을 반환해주는 메소드
	 */
	
	@Override
	public File rename(File originFile) {
		
		// 원본 파일명("bbbb.cccc.aaa.jpg")
		String originName = originFile.getName();
		
		// 수정파일명 : 파일업로드시간(년월일시분초)+ 5자리랜덤값(10000~99999) => 최대한 이름이 겹치지 않게
		// 확장자 : 원본파일명에서 그대로 얻어올 예쩡
		 ...
		 * */
		 ...
		 String changeName = currentTime + ranNum + ext; // 2023022011355012345.jpg
		
		
		 return new File(originFile.getParent() , changeName  );// 원본파일을 수정된 파일명으로 적용시켜서 객체를 반화해줌
